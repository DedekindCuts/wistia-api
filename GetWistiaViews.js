//This is a Google Apps Script, meant to run in a Google Sheet

//A set of functions to write information about Wistia video views to a spreadsheet.
...
//The particular fields that are written to the spreadsheet can be changed, check the WriteToSheet function and check the Wistia API documentation (https://wistia.com/doc/stats-api#events_list) for a list of all fields that are returned.
//Keep in mind that Wistia limits accounts to 100 API calls per minute. Each page of 100 views counts as 1 call, so this shouldn't be an issue since it takes awhile to even write 1 page of results to the sheet.
//This page was extremely useful to me in creating this script: https://www.benlcollins.com/apps-script/beginner-apis/
//I also referenced Wistia's Stats API documentation frequently: https://wistia.com/doc/stats-api
//Feel free to contact me at nick.larsen@getweave.com if you have any questions.
  ...
  if(data == '') {
    ...
    for(i=0; i<data.length; i++){
      if(Object.getOwnPropertyNames(data[i]["conversion_data"]).length != 0) {
        ...
        sheet.getRange(sheet.getLastRow() + 1, 1, 1, 5).setValues([row]);
      }
    }
    return true;
  }
